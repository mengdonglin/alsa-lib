	...
	if (hdr->access & SNDRV_CTL_ELEM_ACCESS_TLV_READWRITE
		...
		switch (tlvt->type) {
			case SNDRV_CTL_TLVT_DB_SCALE:
				scalet = container_of(tlvt, struct snd_tplg_tlv_dbscale_template, hdr);
				scale = &tlv->scale;
...
int tplg_add_mixer(snd_tplg_t *tplg, struct snd_tplg_mixer_template *mixer, struct tplg_elem **e)
{
	...
	elem = tplg_elem_new_common(tplg, NULL, mixer->hdr.name, OBJECT_TYPE_MIXER);
	if (!elem)
		return -ENOMEM;
	...
	//tplg->channel_idx = 0; //remove this?
...
int tplg_add_enum(snd_tplg_t *tplg, struct snd_tplg_enum_template *enum_ctl, struct tplg_elem **e)
{
	struct snd_soc_tplg_enum_control *ec;
	struct tplg_elem *elem;
	int ret, i;
	...
	elem = tplg_elem_new_common(tplg, NULL, enum_ctl->hdr.name, OBJECT_TYPE_ENUM);
	if (!elem)
		return -ENOMEM;
	...
	if (enum_ctl->values != NULL) {
		for (i = 0; i < ec->items; i++) {
			if (enum_ctl->values[i] != NULL)
				memcpy(&ec->values[i * sizeof(int) * SNDRV_CTL_ELEM_ID_NAME_MAXLEN / 4],
					enum_ctl->values[i],
					sizeof(int) * SNDRV_CTL_ELEM_ID_NAME_MAXLEN / 4);
		}
	}
...
int tplg_add_bytes(snd_tplg_t *tplg, struct snd_tplg_bytes_template *bytes_ctl, struct tplg_elem **e)
{
	struct snd_soc_tplg_bytes_control *be;
	struct tplg_elem *elem;
	int ret;
	...
	elem = tplg_elem_new_common(tplg, NULL, bytes_ctl->hdr.name, OBJECT_TYPE_BYTES);
	if (!elem)
		return -ENOMEM;
